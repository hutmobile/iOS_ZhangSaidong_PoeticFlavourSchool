...
open class SweetAlert: UIViewController {
    ...
    var userAction:((_ isOtherButton: Bool) -> Void)?
    let kFont = "Helvetica"
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            let rect = subtitleString.boundingRect(with: CGSize(width: width, height: 0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [kCTFontAttributeName as NSAttributedStringKey: subTitleTextView.font!], context: nil)
        ...
        for button in buttons {
            ...
            buttonRect.append(string.boundingRect(with: CGSize(width: width, height: 0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [kCTFontAttributeName as NSAttributedStringKey: button.titleLabel!.font], context: nil))
        }
...
class CancelAnimatedView: AnimatableView {
    ...
    override required init(frame: CGRect) {
        ...
        t.m34 = 1.0 / -500.0
        t = CATransform3DRotate(t, CGFloat(90.0 * M_2_PI / 180.0), 1, 0, 0)
    ...
    required init(coder aDecoder: NSCoder) {
     ...
     fileprivate var outlineCircle: CGPath {
        ...
        let startAngle: CGFloat = CGFloat((0) / 180.0 * M_2_PI)  //0
        let endAngle: CGFloat = CGFloat((360) / 180.0 * M_2_PI)   //360
    ...
    override func animate() {
        ...
        t2.m34 = 1.0 / -500.0
        t2 = CATransform3DRotate(t2, CGFloat(-M_2_PI), 1, 0, 0)
...
class SuccessAnimatedView: AnimatableView {
    ...
    var path: CGPath {
        let path = UIBezierPath()
        let startAngle: CGFloat = CGFloat((60) / 180.0 * M_2_PI) //60
        let endAngle: CGFloat = CGFloat((200) / 180.0 * M_2_PI)  //190
        path.addArc(withCenter: CGPoint(x: self.frame.size.width/2.0, y: self.frame.size.height/2.0), radius: self.frame.size.width/2.0, startAngle: startAngle, endAngle: endAngle, clockwise: false)
